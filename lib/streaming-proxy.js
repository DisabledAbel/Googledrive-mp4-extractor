...
/**
 * Pipe an upstream Node stream while keeping errors and disconnects contained.
 * Unlike stream.pipeline(), this leaves an untouched response writable so the
 * caller can still send a useful HTTP error before streaming has started.
 */
function pipeUpstreamStream(source, res) {
  return new Promise((resolve, reject) => {
    ...
    const onClose = () => {
      if (!res.writableFinished && !source.destroyed) source.destroy();
      settle();
    };
...
function sendStreamError(res, message, error) {
  if (!res.headersSent && !res.destroyed) {
    // Discard representation headers copied from the now-failed upstream.
    for (const header of [
      'Content-Type',
      'Content-Length',
      'Content-Range',
    ...
    res.end(JSON.stringify({ error: message, detail: error.message }));
  } else if (!res.destroyed) {
    // The status and headers can no longer be changed once a chunk was sent.
    res.destroy();
  }
}
...
class StreamingProxy {
  ...
  async streamDirect(fileId, resourceKey, req, res) {
    try {
      ...
      // Await the entire transfer so failures after the first chunk are handled.
      await pipeUpstreamStream(Readable.fromWeb(response.body), res);
    } catch (error) {
      sendStreamError(res, 'Failed to stream video', error);
    }
  }
}
