...
function pickPathSegment(pathSegments = [], marker) {
  const markerIndex = pathSegments.findIndex((segment) => segment === marker);
  const candidate = markerIndex >= 0 ? pathSegments[markerIndex + 1] : null;
  return candidate && FILE_ID_PATTERN.test(candidate) ? candidate : null;
}

function extractGoogleDriveFileId(raw = '') {
  ...
  const fileIdFromDPath = pickPathSegment(pathSegments, 'd');
  if (fileIdFromDPath) return fileIdFromDPath;

  if (pathSegments[0] === 'drive' && pathSegments[1] === 'folders') {
    const folderId = pathSegments[2] || null;
    if (folderId && FILE_ID_PATTERN.test(folderId)) return folderId;
  }

  const idFromQuery = parsed.searchParams.get('id') || parsed.searchParams.get('file_id');
  if (idFromQuery && FILE_ID_PATTERN.test(idFromQuery)) {
    return idFromQuery;
  }
...
function extractFromUrl(value) {
  try {
    ...
    return {
      fileId,
      resourceKey: parsed.searchParams.get('resourcekey') || parsed.searchParams.get('rk') || null
    };
  } catch {
    return { fileId: null, resourceKey: null };
  }
...
function getResourceKeyFromValue(value = '') {
  ...
  try {
    ...
    const fromQuery = parsed.searchParams.get('resourcekey') || parsed.searchParams.get('rk');
    if (fromQuery) return fromQuery;

    const hashParams = new URLSearchParams((parsed.hash || '').replace(/^#/, ''));
    return hashParams.get('resourcekey') || hashParams.get('rk') || null;
  } catch {
    const fromText = text.match(/[?&#](?:resourcekey|rk)=([a-zA-Z0-9_-]+)/i)?.[1] || null;
    return fromText;
  }
}
...
function extractDriveParams(raw = '') {
  ...
  const fileId = FILE_ID_PATTERN.test(value) ? value : null;
  if (!fileId) return { fileId: null, resourceKey: null };
...
function getCacheKey(fileId, resourceKey) {
  return `${fileId}::${resourceKey || ''}`;
}

function getCachedResolvedUrl(fileId, resourceKey) {
  const key = getCacheKey(fileId, resourceKey);
  const entry = resolvedUrlCache.get(key);
...
function setCachedResolvedUrl(fileId, resourceKey, url) {
  const key = getCacheKey(fileId, resourceKey);
...
function clearCachedResolvedUrl(fileId, resourceKey) {
  resolvedUrlCache.delete(getCacheKey(fileId, resourceKey));
}
...
async function resolveDriveDownloadUrl(fileId, options = {}) {
  ...
  let url = getCachedResolvedUrl(fileId, resourceKey) || candidates[0].toString();
  let attempts = 0;
  ...
  while (attempts < 10) {
    ...
    if (contentType.includes('text/html')) {
      ...
      if (attempts === 1 && url.includes('drive.usercontent.google.com')) {
        clearCachedResolvedUrl(fileId, resourceKey);
    ...
    setCachedResolvedUrl(fileId, resourceKey, url);
    return { url, response };
  }
